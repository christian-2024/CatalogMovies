        ...
        public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "post")] HttpRequest req)
        {
            _logger.LogInformation("Processando a Imagem no Storage");
                ...
                if (file == null || file.Length == 0) {
                    return new BadRequestObjectResult("O arquivo não foi envidado");
                }
                ...
                {
                    await blob.UploadAsync(strem, true);
                }

                _logger.LogInformation($"Arquivo {file.FileName} armazenado com sucesso no container {containerName}"); 

                return new OkObjectResult(new 
                {
                    Message = "Arquivo armazenamento com suscesso",
                    BlobUri = blob.Uri
                });
        }
    }
